...
- **Interactive Gradio Web App**: Local web interface for drag-and-drop dog photo classification, Top-K probability charts, customizable Grad-CAM overlays, breed facts, and execution engine toggles (TensorFlow Metal vs ONNX CoreML).
...
- **Rich CLI Interface**: Single command-line entry point (`dog-breed`) for training, evaluation, single-image inference, ONNX export, and diagnostics.
...
### 1. Prerequisites
Ensure you have Python 3.11+ installed on your Mac. Using [`uv`](https://github.com/astral-sh/uv) is recommended for fast environment resolution.
...
All test stages verify dataset indexing, data augmentation, model compilation, unfreezing, micro-training on Metal GPU, inference, Grad-CAM, and Gradio app construction.
...
- Random Rotations (+/- 12%)
- Random Zoom (+/- 12%)
- Random Translations (+/- 8%)
- Random Contrast adjustments (+/- 10%)

### 2. Regularization & Loss
- **Label Smoothing (epsilon = 0.05)**: Mitigates overconfidence in fine-grained inter-breed visual overlap.
- **L2 Weight Regularization (1e-4)**: Penalizes large weights in dense classification layers.
- **Dropout (p = 0.2)**: Prevents co-adaptation of neurons in classification head.
